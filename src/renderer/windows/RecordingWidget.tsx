...
import { Volume2, VideoIcon, Zap, Minus, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function RecordingWidget() {
  ...
  const handleMinimize = async () => {
    await window.electronAPI?.minimizeRecordingWidget?.();
  };
  ...
  return (
    <div className="w-full h-full bg-gray-900 flex flex-col overflow-hidden select-none" style={{ WebkitAppRegion: 'drag' } as any}>
      {/* Toolbar Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-black" style={{ WebkitAppRegion: 'drag' } as any}>
        <div className="flex items-center gap-3">
          <div className="w-6 h-6 rounded-full bg-red-500/20 flex items-center justify-center animate-pulse">
            <div className="w-2.5 h-2.5 rounded-full bg-red-500" />
          </div>
          <div className="flex-1">
            <h2 className="text-sm font-semibold text-white">Recording Widget</h2>
      ...
      {isExpanded && (
        <>
          ...
          {/* Content Area */}
          <div className="p-4 bg-gray-900 max-h-64 overflow-y-auto flex-1">
            {activeTab === 'notes' ? (
      ...
      <div className="flex items-center justify-between p-3 border-t border-gray-800 bg-black">
        ...
        <button
          ...
          className="px-4 py-1.5 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 text-white rounded-lg font-semibold transition text-xs"
        >
          ■ Stop
        </button>
      </div>
